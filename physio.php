	...
	<div id = "content">
		...
		<div id = "add" class = "panel panel-success">	
			<div class = "panel-heading">
				<label>ADD A PHYSIOTHERAPIST</label>
			...
			<div class = "panel-body">
              ...
              <input type="Password" name="cpassword" class="form-control"  required="required" placeholder="">
				...
				<button type="submit" class = "btn btn-primary" name = "save_physio" ><span class = "glyphicon glyphicon-save"></span> SAVE</button>
				<br />
			</div>	
		...
		<div class = "panel panel-primary">
			...
			<div class = "panel-body">
				...
				<table id = "table" class = "display" width = "100%" cellspacing = "0">
					...
					<?php
					$adm="phy";
						$query = $conn->query("SELECT id,concat(firstname,' ',middlename,' ',lastname) as name,username,phone,idno FROM physiotherapist order by id") or die(mysqli_error());
						while($fetch = $query->fetch_array()){
							$qry = $conn->query("SELECT * from room where physio='$fetch[id]'") or die(mysqli_error());
							$Valid = $qry->num_rows;
					?>
						<tr>
							...
							<td><center>
								<?php if($Valid<=0) { ?>
									<a href="sphysio.php?id=<?php echo $fetch['id']; ?>"><span class="btn btn-primary bg-green"><i class="fa fa-edit"></i> Assign Room </span></a>
								<?php } else { ?>
									<a href="sphysio.php?id=<?php echo $fetch['id']; ?>"><span class="btn btn-warning bg-green"><i class="fa fa-edit"></i> Deprive Room </span></a>
								<?php }  ?>
								<a href="editphysio.php?id=<?php echo $fetch['id']; ?>"><span class="btn btn-success bg-green"><i class="fa fa-edit"></i> Edit </span></a>
							 <a href="deletes.php?id=<?php echo $fetch['id']; ?> &del=<?php echo $adm ?>"><span class="btn btn-danger"><i class="fa fa-trash-o"></i> Delete </span></a></center></td>
