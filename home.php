...
$filter="";
   ...
   { 
        ...
        $date=$_POST['date'];
        $status=$_POST['status'];
        if($physio!="" && $status!="" && $date!="")
        	$filter="where physiotherapist.id='$physio' and app_status.id='$status' and date='$date'";
        else if($status!="" && $date=="" && $physio=="")
        	$filter=" where app_status.id='$status' ";
        else if($date!=""  && $status=="" && $physio=="")
        	$filter=" where date='$date' ";
        else if($physio!="" && $status=="" && $date=="")
        	$filter=" where physiotherapist.id='$physio'";
        else if($physio!="" && $status!="" && $date=="")
        	$filter=" where physiotherapist.id='$physio' and app_status.id='$status' ";
        else if($physio!="" && $status=="" && $date!="")
        	$filter=" where  date='$date' and  physiotherapist.id='$physio' ";
         else if($physio=="" && $status!="" && $date!="")
        	$filter=" where  date='$date' and app_status.id='$status' ";
		...
		<div class = "panel panel-primary">
			...
			<div>
					<form id = "form_user" method = "POST" enctype = "multi-part/form-data">
		              <label for="exampleInputPassword1">Physiotherapist:</label>
		             <select name = "physio"  >
		             	<option value ="">All Physiotherapist</option>
                <?php foreach ($rp1 as $p) {?>
